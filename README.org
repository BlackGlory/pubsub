...
****** 获取JSON Schema
=GET /admin/pubsub/<id>/json-schema=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/pubsub/$id/json-schema"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/pubsub/${id}/json-schema`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
****** 设置JSON Schema
=PUT /admin/pubsub/<id>/json-schema=
...
curl \
  ...
  --data "$JSON_SCHEMA" \
  "http://localhost:8080/admin/pubsub/$id/jsonschema"
#+END_SRC
...
****** 移除JSON Schema
=DELETE /admin/pubsub/<id>/json-schema=
...
****** 获取特定频道的token策略
=GET /admin/pubsub/<id>/token-policies=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/pubsub/$id/token-policies"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fethc(`http://localhost:8080/admin/pubsub/${id}/token-policies`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
****** 设置token策略
=PUT /admin/pubsub/<id>/token-policies/write-token-required=
=PUT /admin/pubsub/<id>/token-policies/read-token-required=
...
curl \
  ...
  "http://localhost:8080/admin/pubsub/$id/token-policies/write-token-required"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/pubsub/${id}/token-policies/write-token-required`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  , 'Content-Type': 'application/json'
...
****** 移除token策略
=DELETE /admin/pubsub/<id>/token-policies/write-token-required=
=DELETE /admin/pubsub/<id>/token-policies/read-token-required=
...
curl \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/pubsub/$id/tokens"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/pubsub/${id}/tokens`, {
  headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
}).then(res => res.json())
...
=PUT /admin/pubsub/<id>/tokens/<token>/write=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/pubsub/$id/tokens/$token/write"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/pubsub/${id}/tokens/${token}/write`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
=DELETE /admin/pubsub/<id>/tokens/<token>/write=
...
****** 为特定频道的token设置read权限
=PUT /admin/pubsub/<id>/tokens/<token>/read=
...
curl \
  --request PUT \
  --header "Authorization: Bearer $ADMIN_PASSWORD" \
  "http://localhost:8080/admin/pubsub/$id/tokens/$token/read"
#+END_SRC

******** fetch
#+BEGIN_SRC js
await fetch(`http://localhost:8080/admin/pubsub/${id}/tokens/${token}/read`, {
  method: 'PUT'
, headers: {
    'Authorization': `Bearer ${adminPassword}`
  }
...
****** 取消特定频道的token的read权限
=DELETE /admin/pubsub/<id>/tokens/<token>/read=
